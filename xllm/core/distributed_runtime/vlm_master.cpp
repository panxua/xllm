...
void VLMMaster::handle_request(std::string prompt,
  ...
  LOG(INFO) << "handler request: calling incre pending requests";
  ...
  threadpool_->schedule([this,
    ...
    LOG(INFO) << "handler request: entering runnable";
    ...
    LOG(INFO) << "handler request: request is to be generated";
    auto request = generate_request(std::move(prompt),
                                    std::move(mm_data),
                                    std::move(sp),
                                    std::move(callback));
    LOG(INFO) << "handler request: request had be generated";
    if (!request) {
      return;
    }
