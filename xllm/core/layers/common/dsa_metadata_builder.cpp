...
void DSAMetadataBuilder::build_dsa_fields(
  ...
  const bool is_acl_graph = params.enable_cuda_graph;
...
void DSAMetadataBuilder::process_swa_group(const torch::Tensor& raw_bt,
  ...
  auto slot_for_position = [&](int32_t seq, int64_t pos) -> int32_t {
    ...
    if (logical_block_idx < 0 || logical_block_idx >= semantic_cols) {
      return -1;
    }
    const int32_t block_id = raw_bt_acc[seq][logical_block_idx];
    if (block_id < 0) {
      return -1;
    }
  ...
  const int32_t current_cols = static_cast<int32_t>(raw_bt.size(1));
  ...
  for (int32_t s = 0; s < batch_size; ++s) {
    ...
    const int32_t retained_cols = std::min(current_cols, max_dst_len);
    for (int32_t logical_col = 0; logical_col < retained_cols; ++logical_col) {
      new_acc[s][logical_col] = old_acc[s][logical_col];
    }
  }
  out_bt = new_bt;
}
...
void DSAMetadataBuilder::build_seq_lengths(const ModelInputParams& params,
  ...
  if (target_device.is_cpu() &&
    ...
    std::vector<int32_t> kv_lens_host(
        params.attention.host.kv_seq_lens.begin(),
        params.attention.host.kv_seq_lens.end());
    std::vector<int32_t> q_lens_host;
    q_lens_host.reserve(batch_size);
    if (static_cast<int32_t>(params.attention.host.q_seq_lens.size()) ==
        batch_size) {
      q_lens_host.assign(params.attention.host.q_seq_lens.begin(),
                         params.attention.host.q_seq_lens.end());
    } else if (params.meta.batch_forward_type.no_decode()) {
    ...
    dsa_metadata.max_query_len = std::max<int64_t>(
        params.meta.q_max_seq_len, vector_max_or_zero(q_lens_host));
    dsa_metadata.max_seq_len = std::max<int64_t>(
        params.meta.kv_max_seq_len, vector_max_or_zero(kv_lens_host));
    return;
  }

  auto kv_lens = params.attention.device.kv_seq_lens;
  if (target_device.is_cpu() &&
      static_cast<int32_t>(params.attention.host.kv_seq_lens.size()) ==
          batch_size) {
    kv_lens =
        torch::tensor(std::vector<int32_t>(
                          params.attention.host.kv_seq_lens.begin(),
                          params.attention.host.kv_seq_lens.end()),
                      int_options);
  ...
  if (static_cast<int32_t>(params.attention.host.q_seq_lens.size()) ==
    ...
    if (target_device.is_cpu()) {
      q_lens =
          torch::tensor(std::vector<int32_t>(
                            params.attention.host.q_seq_lens.begin(),
                            params.attention.host.q_seq_lens.end()),
                        int_options);
