...
DSAttentionImpl::forward(const DSAMetadata& attn_metadata,
  ...
  // overwrites in cmp_cache. The cmp_cache write is deferred to after
  // attention. Decode: write compressed_kv into cmp_cache first, then pass
  // cmp_cache (PA_ND layout).
  ...
  auto [attn_output, output_lse] = xllm::kernel::npu::sparse_attn_sharedkv(
      ...
      /*cmp_block_table=*/
      (compress_ratio_i > 1 && !isprefill) ? as_optional(cmp_block_table)
                                           : c10::nullopt,
